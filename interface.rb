class Interface
  ...
  def info_carriages(c)
  str = "Номер: #{c.number}, тип: #{c.type}"
    if c.type == :passenger
      puts str + ", занято: #{c.taken_seats}, свободно #{c.free_seats}"
    else
      puts str + ", занято: #{c.filled_volume}, свободно #{c.free_volume}"
    end
  end
  ...
  def fill_carriages
    ...
    if my_carriage.class ==  PassengerCarriage
      puts 'Сколько мест будет занято?:'
      gets.chomp.to_i.times { my_carriage.take_seat }
    elsif my_carriage.class ==  CargoCarriage
